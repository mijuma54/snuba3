...
class HeuristicGenerator(object):
    ...
    def apply_heuristics(self, heuristics, primitive_matrix, feat_combos, beta_opt, knn = False):
        """
        ...
        def marginals_to_labels_knn(hf, X, beta):
            pass

        L = np.zeros((np.shape(primitive_matrix)[0], len(heuristics)))
        if knn:
            for i, hf in enumerate(heuristics):
                L[:, i] = marginals_to_labels_knn(hf, primitive_matrix[:, feat_combos[i]], beta_opt[i])
    ...
    def prune_heuristics(self, heuristics, feat_combos, keep=1, knn = False):
        """
        ...
        for i in range(max_cardinality):
            ...
            beta_opt_temp = self.syn.find_optimal_beta(heuristics[i], self.val_primitive_matrix, feat_combos[i],
                                                       self.val_ground)
            L_temp_val = self.apply_heuristics(heuristics[i], self.val_primitive_matrix, feat_combos[i], beta_opt_temp, knn)
    ...
    def run_synthesizer(self, max_cardinality=1, idx=None, keep=1, model='lr', knn = False):
        """
        ...
        sort_idx = self.prune_heuristics(hf, feat_combos, keep, knn)
        for i in sort_idx:
